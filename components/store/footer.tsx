...
export function Footer() {
  ...
  return (
    <footer className="relative mt-24 overflow-hidden bg-linear-to-b from-ink to-ink-soft text-cream">
      {/* Hoa văn nền tinh tế bằng radial gradient champagne */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 opacity-30"
        style={{
          backgroundImage:
            "radial-gradient(circle at 15% 0%, rgba(201, 169, 97, 0.15) 0%, transparent 40%), radial-gradient(circle at 85% 100%, rgba(201, 169, 97, 0.1) 0%, transparent 40%)",
        }}
      />

      {/* Newsletter section nổi bật */}
      <div className="relative border-b border-cream/10">
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-6 px-4 py-12 text-center sm:px-6 lg:flex-row lg:justify-between lg:gap-8 lg:py-16 lg:text-left lg:px-8">
          <div className="max-w-md">
            ...
            <h3 className="mt-3 font-display text-3xl font-light leading-tight sm:text-4xl">
              Đăng ký để nhận ưu đãi <span className="text-gold-gradient">độc quyền</span>
            </h3>
            <p className="mt-3 text-sm text-cream/70">
              Khám phá những bộ sưu tập mới nhất và nhận voucher giảm 5% cho đơn đầu tiên.
            </p>
          </div>
          <form className="w-full max-w-md">
            <div className="flex h-14 items-stretch border border-cream/20 bg-white/5 backdrop-blur transition-colors focus-within:border-champagne">
              <input
                type="email"
                required
              ...
              <button
                type="submit"
                className="bg-champagne px-6 text-xs font-semibold uppercase tracking-[0.2em] text-ink transition-colors hover:bg-champagne-light"
              >
                Đăng ký
              </button>
            </div>
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-[1.3fr_1fr_1fr_1.2fr]">
          ...
          <div>
            ...
            <div aria-hidden className="mt-3 h-px w-12 bg-champagne" />
            <p className="mt-5 max-w-xs text-sm leading-relaxed text-cream/70">
              Đánh thức giác quan qua từng nốt hương. Chúng tôi cung cấp những
              dòng nước hoa cao cấp, chính hãng từ các thương hiệu hàng đầu thế giới.
            </p>
          ...
          {/* Liên hệ */}
          <div>
            <h4 className="text-[11px] font-semibold uppercase tracking-[0.25em] text-champagne">
              Liên hệ
            </h4>
            <div aria-hidden className="mt-3 h-px w-12 bg-champagne/40" />
        ...
        <div className="mt-14 border-t border-cream/15 pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-xs text-cream/60 sm:flex-row">
            <p className="text-center uppercase tracking-[0.2em] sm:tracking-[0.25em]">
              © {year} {siteConfig.name}. All rights reserved.
            </p>
            <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4">
              <span className="italic text-cream/50">Thiết kế bởi {siteConfig.name} Team</span>
              <span className="rounded border border-cream/20 px-2.5 py-1 text-[10px] font-semibold tracking-[0.18em]">
                VNPAY
              </span>
              <span className="rounded border border-cream/20 px-2.5 py-1 text-[10px] font-semibold tracking-[0.18em]">
                MOMO
...
function SocialIcon({
  ...
  return (
    <a
      ...
      aria-label={label}
      className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-cream/20 text-cream/80 transition-all duration-300 ease-luxe hover:border-champagne hover:bg-champagne hover:text-ink hover:scale-110"
    >
      <svg
        viewBox="0 0 24 24"
        className="h-4 w-4"
